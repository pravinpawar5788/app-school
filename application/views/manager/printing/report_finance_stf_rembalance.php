...
<div class="content" >
	...
	<p class="text-center">
		Staff Remaining Balance Report
				    ...
				    <table class="table table-sm">
				        <thead>
				            <tr>
				                ...
				                <th class="font-weight-semibold text-center" width="10%">Staff ID</th>
				                <th class="font-weight-semibold text-center">Staff Name</th>
				                <th class="font-weight-semibold text-center">Staff Mobile</th>
				                <th class="font-weight-semibold text-center" width="20%">Remaining Balance</th>
				            </tr>
				        </thead>
				        <tbody>
							...
							$i=0;
							$total_amount=0;
							$pay_filter=array('campus_id'=>$this->CAMPUSID,'status <>'=>$this->stf_pay_voucher_m->STATUS_PAID);
							if(!empty($month) ){$pay_filter['month']=$month;}
							if(!empty($year)){$pay_filter['year']=$year;}
							foreach($staff as $usr){
								$amount=0;
								$pay_filter['staff_id']=$usr['mid'];
								$vouchers=$this->stf_pay_voucher_m->get_rows($pay_filter);
								if(count($vouchers)<1){continue;}
								foreach($vouchers as $voucher){
									$amount+=$this->stf_pay_entry_m->get_voucher_amount($voucher['voucher_id'],'',$this->CAMPUSID);
								}
	...
	<?php print $this->config->item('app_print_code');?>5014 <span class="page-footer-right"><?php print ucwords($this->SETTINGS[$this->system_setting_m->_ORG_NAME]);?></span>
	</div>	
