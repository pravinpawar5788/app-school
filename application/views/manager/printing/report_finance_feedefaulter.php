...
isset($form['month']) && !empty($form['month']) ? $month=$form['month'] : $month='';
isset($form['year']) && !empty($form['year']) ? $year=$form['year'] : $year='';
...
$params['orderby']='class_id ASC, roll_no ASC, mid ASC';
$params['select']='mid,name,image,student_id, class_id,section_id,mobile,guardian_name, guardian_mobile,roll_no';
...
<div class="content" >
	      ...
	      <div class="page" style="line-height: 1;">
			    ...
			    <table class="table table-sm">
			        <thead>
			            <tr>
			                ...
			                <th class="font-weight-semibold text-center" width="17%">Class</th>
			                <th class="font-weight-semibold text-center" width="18%">Student</th>
			                <th class="font-weight-semibold text-center">Guardian</th>
			                <th class="font-weight-semibold text-center" width="10%">Roll No</th>
			                <th class="font-weight-semibold text-center" width="15%">Due Amount</th>
			            </tr>
			        </thead>
			        <tbody>
						...
						$i=0;
						$total_amount=0;
						$fee_filter=array('campus_id'=>$this->CAMPUSID);
						$fee_params=array('select'=>'mid,voucher_id,student_id');
						if(isset($form['inc_partial'])){
							$fee_filter['status <>']=$this->std_fee_voucher_m->STATUS_PAID;
						}else{
							$fee_filter['status']=$this->std_fee_voucher_m->STATUS_UNPAID;										
						}
						if(!empty($month) ){$fee_filter['month']=$month;}
						if(!empty($year)){$fee_filter['year']=$year;}
						foreach($students as $usr){
							$amount=0;
							$fee_filter['student_id']=$usr['mid'];
							$vouchers=$this->std_fee_voucher_m->get_rows($fee_filter,$fee_params);
							if(count($vouchers)<1){continue;}
							foreach($vouchers as $voucher){
								$amount+=$this->std_fee_entry_m->get_voucher_amount($voucher['voucher_id'],'',$this->CAMPUSID);
							}
						...
						?>
				            <tr>
				                ...
				                <td class="text-center"><?php print $usr['guardian_name'].' ('.$usr['guardian_mobile'].')';?></td>
			...
			<div class="row">
				<div class="col-sm-3 border-solid text-center">
					<span class="text-bold">Total Receiveable Amount:</span>
				</div>
	...
	<?php print $this->config->item('app_print_code');?>5009 <span class="page-footer-right"><?php print ucwords($this->SETTINGS[$this->system_setting_m->_ORG_NAME]);?></span>
	</div>	
