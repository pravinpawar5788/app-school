...
<div class="content" >
	...
	<p class="text-center">
		Pending Fee Vouchers Report
		<?php if(!empty($month)){ print '- '.month_string($month);} ?>
		<?php if(!empty($year)){ print ' , '.$year;} ?>
		. List of all vouchers not yet paid or partially paid by students
	</p>
			    ...
			    <table class="table table-sm">
			        <thead>
			            <tr>
			                ...
			                <th class="font-weight-semibold text-center" width="8%">Amount</th>
			            </tr>
			        </thead>
			        <tbody>
						...
						foreach($vouchers as $row){
							...
							$amount=$this->std_fee_entry_m->get_voucher_amount($row['voucher_id'],'',$this->CAMPUSID);
	...
	<?php print $this->config->item('app_print_code');?>5010 <span class="page-footer-right"><?php print ucwords($this->SETTINGS[$this->system_setting_m->_ORG_NAME]);?></span>
	</div>	
