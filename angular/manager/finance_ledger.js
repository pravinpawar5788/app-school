app.controller("mozzCtrl", function($scope,$http,$timeout,$log,$window,$sce,$interval,config,hotkeys,Pubnub){
	...
	$scope.filter={account:'',month:'',year:'',period:''};
    ...
    $scope.loadRows=function (){
		var data={'search':$scope.searchText,'page':config.currentPage,'period':$scope.filter.period,'month':$scope.filter.month,'year':$scope.filter.year,'debit_account_id':$scope.filter.debit_account,'credit_account_id':$scope.filter.credit_account,'sortby':$scope.sortString};
		config.postConfig.params=data;
		$scope.disableButtons();
		$http.get($scope.ajax_url+'filterLedger',config.postConfig).then(function(response){
			$scope.enableButtons();
			$scope.responseData=response.data;
			config.responseNumRows=$scope.responseData.total_rows;
			$scope.validatePagination();
	...
	$scope.saveRow=function (){	
		...
		swal({title:"Please select debit account",type:'info',showConfirmButton:false,timer:2000});
		...
		}else if($scope.entry.credit_account===$scope.entry.account){
		swal({title:"Debit and Credit account cannot be same. Please select difference credit account",type:'info',showConfirmButton:false,timer:3500});
		...
		swal({title:"Please wait for existing command.",type:'info',showConfirmButton:false,timer:2000});
			...
			data+='&date='+encodeURIComponent($scope.entry.date)+'&credit_account='+encodeURIComponent($scope.entry.credit_account)+'&description='+encodeURIComponent($scope.entry.description);
			$http.post($scope.ajax_url+'addLedgerRecord',data,config.postConfig).then(function(response){
				$scope.enableButtons();
	...
	$scope.updateRow=function (){	
		...
		swal({title:"Please select date",type:'info',showConfirmButton:false,timer:2000});
		}else if($scope.selectedRow.is_manual< 1){
		swal({title:"System entry can not be changed.",type:'info',showConfirmButton:false,timer:2000});
		}else if(config.btnClickedSave===true){
		swal({title:"Please wait for existing command.",type:'info',showConfirmButton:false,timer:2000});
		}else{				
			$scope.disableButtons();
			...
			$http.post($scope.ajax_url+'updateLedgerRecord',data,config.postConfig).then(function(response){
				$scope.enableButtons();
				var msg=response.data.message;
				if(response.data.error === true){
				swal({title:msg,type:'error',showConfirmButton:false,timer:2500});
	...
	$scope.delRow=function (row){	
		swal({
			...
			if(result.value){
				...
				$http.post($scope.ajax_url+'deleteLedgerRecord',data,config.postConfig).then(function(response){
