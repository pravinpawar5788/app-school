app.controller("mozzCtrl", function($scope,$http,$timeout,$log,$window,$sce,$interval,config,hotkeys){
	...
	$scope.ajax_url=''+config.ajaxModule+'support/';
	...
	$scope.sortString='';
	///////////////////////////////
	$scope.filter={status:'',type:''};
	$scope.entry={title:'',message:'',type:''};
    $scope.students={};
    $scope.promise={};
	$scope.message='';
	$scope.usrid='';
    ...
    $scope.loadRows=function (){
		var data={'search':$scope.searchText,'page':config.currentPage,'type':$scope.filter.type,'sortby':$scope.sortString};
	...
	$scope.saveRow=function (){	
		...
		data+='&type='+encodeURIComponent($scope.entry.type);
