app.controller("mozzCtrl", function($scope,$http,$timeout,$log,$window,$sce,$interval,config,hotkeys,Pubnub){
    ...
    $scope.loadRows=function (){
		...
		$http.get($scope.ajax_url+'filterLiability',config.postConfig).then(function(response){
			$scope.enableButtons();
	...
	$scope.saveRow=function (){	
		...
		swal({title:"Please wait for existing command.",type:'info',showConfirmButton:false,timer:2000});
			...
			$http.post($scope.ajax_url+'addLiability',data,config.postConfig).then(function(response){
				$scope.enableButtons();
	...
	$scope.updateRow=function (){	
		...
		swal({title:"Please wait for existing command.",type:'info',showConfirmButton:false,timer:2000});
			...
			$http.post($scope.ajax_url+'updateLiability',data,config.postConfig).then(function(response){
				$scope.enableButtons();
				var msg=response.data.message;
				if(response.data.error === true){
				swal({title:msg,type:'error',showConfirmButton:false,timer:2500});
	...
	$scope.delRow=function (row){	
		swal({
			...
			if(result.value){
				...
				$http.post($scope.ajax_url+'deleteLiability',data,config.postConfig).then(function(response){
