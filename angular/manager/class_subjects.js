app.controller("mozzCtrl", function($scope,$http,$timeout,$log,$window,$sce,$interval,config,hotkeys,Pubnub){
	...
	$scope.entry={name:'',code:'',total_marks:'',passing_percentage:'',chapters:'',description:'',section:'',homework:'',area:'',term:''};
    $scope.promise={};
	$scope.message='';
	$scope.class={};
	$scope.pid='';
	...
	$scope.updateRow=function (){	
		...
		$scope.disableButtons();
		var data='rid='+$scope.selectedRow.mid+'&roll_no='+encodeURIComponent($scope.selectedRow.roll_no)+'&section='+encodeURIComponent($scope.selectedRow.section);
    ...
    $scope.saveHomework=function (){	
		...
		$scope.disableButtons();
		var data='rid='+$scope.selectedRow.mid+'&section='+encodeURIComponent($scope.entry.section)+'&homework='+encodeURIComponent($scope.entry.description);
		// data+='&anual_increment='+encodeURIComponent($scope.staff.anual_increment);
    ...
    $scope.eraseExamData=function (){	
    	if($scope.entry.area.length< 1){
		swal({title:"Please select target area",type:'info',showConfirmButton:false,timer:2000});
        }else if(config.btnClickedSave===true){
		swal({title:"Please wait for existing command.",type:'info',showConfirmButton:false,timer:2000});
        }else{
		$scope.disableButtons();
		var data='rid='+$scope.selectedRow.mid+'&area='+encodeURIComponent($scope.entry.area)+'&term='+encodeURIComponent($scope.entry.term);
		// data+='&anual_increment='+encodeURIComponent($scope.staff.anual_increment);
		$http.post($scope.ajax_url+'eraseExamData',data,config.postConfig).then(function(response){
			$scope.enableButtons();
			var msg=response.data.message;
			if(response.data.error === true){
			swal({title:msg,type:'error',showConfirmButton:false,timer:2500});
	...
	///////autoload functions////////////////////
	// $scope.loadRows();
	$timeout(function(){$scope.loadSubjects();},1000); 
