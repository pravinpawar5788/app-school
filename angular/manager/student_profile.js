app.controller("mozzCtrl", function($scope,$http,$timeout,$log,$window,$sce,$interval,config,hotkeys,Pubnub){
	...
	$scope.advance={title:'',amount:'',create_voucher:'',type:'', add:true};
	...
	$scope.concession={type_id:'',amount:'',type:'', add:true};
    $scope.promise={};
	$scope.message='';
	$scope.member={};
	$scope.usrid='';
	...
	$scope.saveAdvance=function (){	
		...
		if($scope.advance.add===true){	
			...
			data +='&type='+encodeURIComponent($scope.advance.type);
			data +='&create_voucher='+encodeURIComponent($scope.advance.create_voucher);
			$http.post($scope.ajax_url+'addAdvance',data,config.postConfig).then(function(response){
				...
				swal({title:msg,type:'success',showConfirmButton:false,timer:2000});
				$scope.advance.title='';$scope.advance.amount='';
				$scope.loadAdvance();
				}
			},function(response){
				$scope.enableButtons();
    ...
    $scope.loadConcession=function (){
		var data={'search':$scope.searchText,'page':config.currentPage,'sortby':$scope.sortString,'rid':$scope.member.mid};
		config.postConfig.params=data;
		$scope.disableButtons();
		$http.get($scope.ajax_url+'filterConcession',config.postConfig).then(function(response){
			$scope.enableButtons();
			$scope.responseData=response.data;
			config.responseNumRows=$scope.responseData.total_rows;
			$scope.validatePagination();
	...
	$scope.saveConcession=function (){	
    	if($scope.concession.type_id.length< 1){
		swal({title:"Please select concession category",type:'info',showConfirmButton:false,timer:2000});
        }else if($scope.concession.type.length< 1){
		swal({title:"Please select concession type",type:'info',showConfirmButton:false,timer:2000});
        }else if($scope.concession.amount< 1){
		swal({title:"Please provide concession value",type:'info',showConfirmButton:false,timer:2000});
        }else if(config.btnClickedSave===true){
		swal({title:"Please wait for existing command.",type:'info',showConfirmButton:false,timer:2000});
        }else{
		$scope.disableButtons();
		if($scope.concession.add===true){	
			//add data
			var data='rid='+encodeURIComponent($scope.member.mid)+'&type='+encodeURIComponent($scope.concession.type)+'&amount='+encodeURIComponent($scope.concession.amount);
			data +='&type_id='+encodeURIComponent($scope.concession.type_id);
			$http.post($scope.ajax_url+'addConcession',data,config.postConfig).then(function(response){
				$scope.enableButtons();
				var msg=response.data.message;
				if(response.data.error === true){
				swal({title:msg,type:'error',showConfirmButton:false,timer:4500});
				...
				swal({title:msg,type:'success',showConfirmButton:false,timer:2000});
				$scope.concession.amount='';
				$scope.loadConcession();
				}
			},function(response){
				$scope.enableButtons();
				swal({title:config.globalError,type:'error',showConfirmButton:false,timer:2500});
	...
	$scope.delConcession=function (row){	
		swal({
			title: 'Are you sure?',
			text: "Terminate concession of this member. ",
			type: 'warning',
			showCancelButton: true,
			confirmButtonText: '<strong>Yes, Terminate it <i class="icon-circle-right2 ml-2"></i></strong>',
			cancelButtonText: 'No, Go Back <i class="icon-circle-left2 ml-2"></i>',
			...
			if(result.value){
				var msg='';
				var data='rid='+row.mid;
				$http.post($scope.ajax_url+'deleteConcession',data,config.postConfig).then(function(response){
					msg=response.data.message;
					if(response.data.error===true){
						swal({title:msg,type:'error',showConfirmButton:false,timer:2500});
						$log.error(response);	
    ...
    $scope.loadVouchers=function (){
		...
		config.postConfig.params=data;
		$scope.disableButtons();
		$http.get($scope.ajax_url+'filterFeeVouchers',config.postConfig).then(function(response){
			$scope.enableButtons();
			$scope.responseData=response.data;
			config.responseNumRows=$scope.responseData.total_rows;
			$scope.validatePagination();
	...
	$scope.saveAcademic=function (){	
    	if($scope.academic.session.length< 1){
		swal({title:"Please enter session",type:'info',showConfirmButton:false,timer:2000});
        }else if($scope.academic.class.length< 1){
		swal({title:"Please enter class",type:'info',showConfirmButton:false,timer:2000});
        }else if($scope.academic.total_marks.length< 1){
		swal({title:"Please provide a total marks",type:'info',showConfirmButton:false,timer:2000});
        }else if($scope.academic.obtained_marks.length< 1){
		swal({title:"Please provide a obtained marks",type:'info',showConfirmButton:false,timer:2000});
        }else if(config.btnClickedSave===true){
		swal({title:"Please wait for existing command.",type:'info',showConfirmButton:false,timer:2000});
        }else{
		$scope.disableButtons();
		if($scope.academic.add===true){	
			//add data
			var data='rid='+encodeURIComponent($scope.member.mid)+'&session='+encodeURIComponent($scope.academic.session)+'&class='+encodeURIComponent($scope.academic.class);
			data+='&roll_number='+encodeURIComponent($scope.academic.roll_number)+'&total_marks='+encodeURIComponent($scope.academic.total_marks)+'&obtained_marks='+encodeURIComponent($scope.academic.obtained_marks);
			data+='&status='+encodeURIComponent($scope.academic.status);
			$http.post($scope.ajax_url+'addQual',data,config.postConfig).then(function(response){
				...
				swal({title:msg,type:'success',showConfirmButton:false,timer:2000});
				$scope.academic.session='';$scope.academic.class='';$scope.academic.status='';
				$scope.academic.roll_number='';$scope.academic.total_marks='';$scope.academic.obtained_marks='';
				$scope.loadAcademic();
				}
			},function(response){
				$scope.enableButtons();
			...
			//update data
			var data='rid='+encodeURIComponent($scope.academic.mid)+'&session='+encodeURIComponent($scope.academic.session)+'&class='+encodeURIComponent($scope.academic.class);
			data+='&roll_number='+encodeURIComponent($scope.academic.roll_number)+'&total_marks='+encodeURIComponent($scope.academic.total_marks)+'&obtained_marks='+encodeURIComponent($scope.academic.obtained_marks);
			data+='&status='+encodeURIComponent($scope.academic.status);
	...
	$scope.delAcademic=function (row){	
		swal({
			title: 'Are you sure?',
			text: "Remove academic record("+row.session+") of this member. ",
			type: 'warning',
			showCancelButton: true,
	...
	$scope.loadResults=function (){
		...
		config.postConfig.params=data;
		$scope.disableButtons();
		$http.get($scope.ajax_url+'filterResults',config.postConfig).then(function(response){
			$scope.enableButtons();
			$scope.responseData=response.data;
			config.responseNumRows=$scope.responseData.total_rows;
			$scope.validatePagination();
