app.controller("mozzCtrl", function($scope,$http,$timeout,$log,$window,$sce,$interval,config,hotkeys,Pubnub){
	...
	$scope.entry={title:'',amount:'',account:'',date:'',description:'',debit_account:''};
    ...
    $scope.loadRows=function (){
		...
		$http.get($scope.ajax_url+'filterIncome',config.postConfig).then(function(response){
			$scope.enableButtons();
	...
	$scope.saveRow=function (){	
		...
		}else if($scope.entry.debit_account.length< 1){
		...
		swal({title:"Please wait for existing command.",type:'info',showConfirmButton:false,timer:2000});
			...
			data+='&debit_account='+encodeURIComponent($scope.entry.debit_account);
			$http.post($scope.ajax_url+'addIncome',data,config.postConfig).then(function(response){
				$scope.enableButtons();
	...
	$scope.updateRow=function (){	
		...
		swal({title:"Please wait for existing command.",type:'info',showConfirmButton:false,timer:2000});
			...
			$http.post($scope.ajax_url+'updateIncome',data,config.postConfig).then(function(response){
				$scope.enableButtons();
				var msg=response.data.message;
				if(response.data.error === true){
				swal({title:msg,type:'error',showConfirmButton:false,timer:2500});
	...
	$scope.delRow=function (row){	
		swal({
			...
			if(result.value){
				...
				$http.post($scope.ajax_url+'deleteIncome',data,config.postConfig).then(function(response){
