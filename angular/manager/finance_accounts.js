app.controller("mozzCtrl", function($scope,$http,$timeout,$log,$window,$sce,$interval,config,hotkeys,Pubnub){
	...
	$scope.filter={type:'',month:'',year:'',period:''};
	$scope.entry={title:'',type:'',key:'',is_current_asset:''};
    ...
    $scope.loadRows=function (){
		var data={'search':$scope.searchText,'page':config.currentPage,'type':$scope.filter.type,'period':$scope.filter.period,'sortby':$scope.sortString};
		config.postConfig.params=data;
		$scope.disableButtons();
		$http.get($scope.ajax_url+'filterAccounts',config.postConfig).then(function(response){
			$scope.enableButtons();
	...
	$scope.saveRow=function (){	
		...
		swal({title:"Please select account type",type:'info',showConfirmButton:false,timer:2000});
		...
		swal({title:"Please wait for existing command.",type:'info',showConfirmButton:false,timer:2000});
			...
			$scope.disableButtons();
			var data='title='+encodeURIComponent($scope.entry.title)+'&type='+encodeURIComponent($scope.entry.type);
			data+='&is_current_asset='+encodeURIComponent($scope.entry.is_current_asset);
			$http.post($scope.ajax_url+'addAccount',data,config.postConfig).then(function(response){
				$scope.enableButtons();
	...
	$scope.updateRow=function (){	
		...
		swal({title:"Please wait for existing command.",type:'info',showConfirmButton:false,timer:2000});
			...
			data+='&is_current_asset='+encodeURIComponent($scope.selectedRow.is_current_asset);
			$http.post($scope.ajax_url+'updateAccount',data,config.postConfig).then(function(response){
				$scope.enableButtons();
				var msg=response.data.message;
				if(response.data.error === true){
				swal({title:msg,type:'error',showConfirmButton:false,timer:2500});
	...
	$scope.delRow=function (row){	
		swal({
			...
			if(result.value){
				...
				$http.post($scope.ajax_url+'deleteAccount',data,config.postConfig).then(function(response){
	...
	$scope.resetAccounts=function (){	
		if($scope.entry.key.length< 1){
		swal({title:"Please enter data reset password",type:'info',showConfirmButton:false,timer:2000});
		}else if(config.btnClickedSave===true){
		swal({title:"Please wait for existing command.",type:'info',showConfirmButton:false,timer:2000});
		}else{				
			$scope.disableButtons();
			var data='key='+encodeURIComponent($scope.entry.key);
			$http.post($scope.ajax_url+'resetCampusAccounts',data,config.postConfig).then(function(response){
				$scope.enableButtons();
				var msg=response.data.message;
				if(response.data.error === true){
				swal({title:msg,type:'error',showConfirmButton:false,timer:2500});
