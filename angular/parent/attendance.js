...
app.controller("mozzCtrl", function($scope,$http,$timeout,$log,$window,$sce,$interval,config,hotkeys){
	...
	$scope.sortString='';
	///////////////////////////////
	$scope.filter={class_id:'',date:'',target:'',isHoliday:false};
	...
	$scope.loadStudents=function (){
		...
		swal({title:"Please choose date",type:'info',showConfirmButton:false,timer:2000});
			...
			var data={'class_id':$scope.filter.class_id,'date':$scope.filter.date};
			config.postConfig.params=data;
			$scope.disableButtons();
	...
	$scope.markClassAttendance=function (status){	
		swal({
			...
			if(result.value){
				var msg='';
				$scope.blockPageUI();
				var data='date='+$scope.filter.date+'&class_id='+$scope.filter.class_id+'&status='+status;
	...
	//mark holiday
	$scope.markHoliday=function (){	
		swal({
			title: 'Are you sure?',
			text: "Mark ("+$scope.filter.date+") as holiday for all staff and students.",
			type: 'warning',
			showCancelButton: true,
			confirmButtonText: '<strong>Yes, Continue <i class="icon-circle-right2 ml-2"></i></strong>',
			cancelButtonText: 'No, Go Back <i class="icon-circle-left2 ml-2"></i>',
			...
			if(result.value){
				...
				var data='date='+$scope.filter.date;
				$http.post($scope.ajax_url+'markHoliday',data,config.postConfig).then(function(response){
					msg=response.data.message;
					if(response.data.error===true){
						swal({title:msg,type:'error',showConfirmButton:false,timer:2500});
						$log.error(response);	
