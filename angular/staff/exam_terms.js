app.controller("mozzCtrl", function($scope,$http,$timeout,$log,$window,$sce,$interval,config,hotkeys){
	...
	$scope.entry={name:'',start_date:'',end_date:'',type:''};
    $scope.promise={};
	$scope.message='';
	$scope.usrid='';
    ...
    $scope.loadRows=function (){
		...
		$http.get($scope.ajax_url+'filterTerms',config.postConfig).then(function(response){
			$scope.enableButtons();
	...
	$scope.saveRow=function (){	
		...
		swal({title:"Please enter term name",type:'info',showConfirmButton:false,timer:2000});
        }else if($scope.entry.start_date.length< 1){
		swal({title:"Please select from date",type:'info',showConfirmButton:false,timer:2000});
		...
		$scope.disableButtons();
		var data='name='+encodeURIComponent($scope.entry.name)+'&start_date='+encodeURIComponent($scope.entry.start_date)+'&end_date='+encodeURIComponent($scope.entry.end_date);
		$http.post($scope.ajax_url+'addTerm',data,config.postConfig).then(function(response){
			$scope.enableButtons();
			...
			$scope.entry.name='';$scope.entry.start_date='';$scope.entry.end_date='';
			$scope.loadRows();
			}
		},function(response){
			$scope.enableButtons();
	...
	$scope.updateRow=function (){	
		...
		swal({title:"Please enter term name",type:'info',showConfirmButton:false,timer:2000});
        }else if($scope.selectedRow.start_date.length< 1){
		swal({title:"Please select from date",type:'info',showConfirmButton:false,timer:2000});
        }else if(config.btnClickedSave===true){
		swal({title:"Please wait for existing command.",type:'info',showConfirmButton:false,timer:2000});
        }else{
		$scope.disableButtons();
		var data='rid='+$scope.selectedRow.mid+'&name='+encodeURIComponent($scope.selectedRow.name)+'&start_date='+encodeURIComponent($scope.selectedRow.start_date);
		data+='&end_date='+encodeURIComponent($scope.selectedRow.end_date);
		$http.post($scope.ajax_url+'updateTerm',data,config.postConfig).then(function(response){
			$scope.enableButtons();
			var msg=response.data.message;
			if(response.data.error === true){
			swal({title:msg,type:'error',showConfirmButton:false,timer:2500});
	...
	$scope.delRow=function (row){	
		swal({
			title: 'Are you sure?',
			text: "Delete term ("+row.name+"). All entered result for this term will also be removed. You can not recover it later.!",
			type: 'warning',
			...
			if(result.value){
				...
				$http.post($scope.ajax_url+'deleteTerm',data,config.postConfig).then(function(response){
